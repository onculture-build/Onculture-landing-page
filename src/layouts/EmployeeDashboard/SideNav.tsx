...
const SideNav = () => {
  ...
  const getCourseModule = async () => {
    ...
    const { data } = await axios.get(`${process.env.REACT_APP_API_PROXY}/subscription/getCourse/${profileInfo.user.courses[0].sanityId}`)
  ...
  const getCourseLesson = async () => {
    ...
    await axios.get(`${process.env.REACT_APP_API_PROXY}/subscription/getLesson/${progress.lessonId}`).then((res) => {
