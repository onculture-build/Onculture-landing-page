  ...
  if (!profileInfo.company[0].employees.length) {
    return (
      <div>
        <EmptyState
          imag={emptStateBio}
          text="Looks like you haven't added any employee. Click <a href=# classname='marklink'>here</a> to
    ...
    return (
      <div className={boardEmployeeStyle.employeeMain}>
        ...
        <div>
          ...
          {searchTerm ? (
            <Table data={searchResults} rowsPerPage={6} />
          ) : (
            <Table data={profileInfo.company[0].employees} rowsPerPage={6} />
          )}
        </div>
      </div>
    );
